...
2. Locate where the feature must land in the codebase. **If `{{CODEBASE_MAP_PATH}}` is not `none`, read it first** — it points you at the modules in this feature's blast radius, the utilities to reuse (don't reinvent them), and the landmines to avoid. Then read and search the surrounding code to understand it, and **read the project's `AGENTS.md`, `CLAUDE.md`, and/or `.github/copilot-instructions.md` if present and follow their conventions** (style, structure, libraries, testing) — do not assume they were auto-loaded into your context. Do NOT modify yet.
...
6. **No subagents.** You cannot dispatch subagents (no Codex subagent tool / `Agent` / `task`). You are a leaf in the mission graph.
...
This prompt is written in **action language** — use your runtime's equivalents: read a file (Codex `exec_command` with `sed`/`rg`/`find`; Claude `Read`; Copilot `view`), create or edit a file (Codex `apply_patch`; Claude `Write`/`Edit`; Copilot `create`/`apply_patch`), run a shell command (Codex `exec_command`; Claude `Bash`; Copilot `bash`), search contents (`rg`/`Grep`), find files (`rg --files`/`Glob`/`glob`), plus any project-specific tools available in the parent environment. Full map: `references/codex-tools.md`, `references/claude-tools.md`, `references/copilot-tools.md`.

You **cannot dispatch subagents** (no Codex subagent tool / `Agent` / `task`) — you are a leaf. You may write only the handoff at `{{HANDOFF_PATH}}`; you do NOT have permission to modify `state.json` or any other file under `{{MISSION_DIR}}`.
